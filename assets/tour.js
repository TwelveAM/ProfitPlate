...
(function () {
  ...
  const STEPS = [
    ...
    // SETTINGS – Behaviour
    {
      ...
      panelPlacement: "bottom",
      // Nudge the highlight down a bit so it hugs only the Behaviour box
    ...
    // SETTINGS – Data & local storage
    {
      ...
      // Tiny tweak up so the highlight fits the whole Data & local storage section nicely
  ...
  function renderCurrentStep() {
    ...
    if (targetRect) {
      ...
      const top =
        targetRect.top - padding + (offset.top || 0);
      const left =
        targetRect.left - padding + (offset.left || 0);
      const width =
        targetRect.width + padding * 2 - (offset.left || 0) - (offset.right || 0);
      const height =
        targetRect.height + padding * 2 - (offset.top || 0) - (offset.bottom || 0);
  ...
  function maybeResumeTourFromState() {
    ...
    if (!step || step.page !== PAGE_ID) {
      // State belongs to a different page; do nothing here.
      return;
    }
